...
class pinjie(object):
    ...
    def denglushuju(self, zhongduanhao):
        zhongduanhao = zhongduanhao.zfill(16)
        data = "78781101" + zhongduanhao + "200032000001432D0D0A"
        dlsj = self.jiaoyanV3(data)
        return dlsj

    def weizhishuju(self, weidu, jingdu, sudu, zhuangtai, baojing, xingcheng):
        # 时间  hours=-8 减8小时  days=1 加一天  minutes 分钟操作
        ti1 = (datetime.datetime.now() + datetime.timedelta(hours=-8)).strftime("%Y%m%d%H%M%S")
        ti2 = str(ti1)
        ti3 = 2 * '' + ti2[2:]
        ti4 = list(ti3)
        n = ti4[0] + ti4[1]
        ...
        f = ti4[8] + ti4[9]
        m = ti4[10] + ti4[11]
        n1 = str(hex(int(n)))[2:].zfill(2)
        y1 = str(hex(int(y)))[2:].zfill(2)
        r1 = str(hex(int(r)))[2:].zfill(2)
        s1 = str(hex(int(s)))[2:].zfill(2)
        f1 = str(hex(int(f)))[2:].zfill(2)
        m1 = str(hex(int(m)))[2:].zfill(2)
        nyrsfm = n1 + y1 + r1 + s1 + f1 + m1
        sj = nyrsfm.upper()
        # 经纬度
        aa = int(round(float(weidu) * 60 * 30000))
        bb = int(round(float(jingdu) * 60 * 30000))
        cc = str(hex(aa))
        dd = str(hex(bb))
        ee1 = cc[2:]
        ee = ee1.zfill(8)
        ...
        jd = ff.upper()
        # 速度
        sd1 = str(hex(int(sudu)))
        sd2 = sd1[2:]
        sd = sd2.zfill(2)
        # 状态
        if zhuangtai.strip() == "不定位":
            zt = "0400"
        else:
            zt = "1400"
        # 行程
        if xingcheng.strip() == "开始":
            xc = "0A"
        elif xingcheng.strip() == "结束":
            xc = "09"
        else:
            xc = "00"
        # 报警
        if baojing.find("震动") != -1:
            wzsj = "78782526" + sj + "C4" + wd + jd + sd + zt + "0801CC00262C001114" + "8D06040302" + "004093E60D0A"
        elif baojing.find("超速") != -1:
            wzsj = "78782526" + sj + "C4" + wd + jd + sd + zt + "0801CC00262C001114" + "7706040602" + "004093E60D0A"
        elif baojing.find("剪线") != -1:
            wzsj = "78782526" + sj + "C4" + wd + jd + sd + zt + "0801CC00262C001114" + "D706040202" + "004093E60D0A"
        else:
            asdg = "00000600C7FFFFFFFF000C00B289860404191790800006000400CE111C0004A5450D0A"
            wzsj = "78783E22" + sj + "C7" + wd + jd + sd + zt + "01CC00262C0010F501" + xc + asdg
        wzsj = self.jiaoyanV3(wzsj)
        return wzsj
